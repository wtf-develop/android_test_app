 ...
 */
public class MyHurlStack extends BaseHttpStack {

    public interface MyUrlRewriter {
        /**
     ...
     */
    public interface UrlRewriter extends MyUrlRewriter {
    }
    ...
    public HttpResponse executeRequest(Request<?> request, Map<String, String> additionalHeaders)
        ...
        try {
            ...
            int protect = 4; //hardcoded max redirections
            while ((responseCode == 307) || (responseCode == HttpURLConnection.HTTP_MOVED_TEMP) || (responseCode == HttpURLConnection.HTTP_MOVED_PERM) || (responseCode == HttpURLConnection.HTTP_SEE_OTHER)) {
                protect--;
                if (protect < 1) break;
                String newUrl = connection.getHeaderField("Location");
                connection.disconnect();
                if (mUrlRewriter != null) {
                    String rewritten = mUrlRewriter.rewriteUrl(newUrl);
                    if (rewritten == null) {
                        throw new IOException("URL blocked by rewriter: " + url);
                    }
                    newUrl = rewritten;
                }
                connection = openConnection(new URL(newUrl), request);
                connection.setInstanceFollowRedirects(true);
